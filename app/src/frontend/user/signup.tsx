...
export const SignUp: React.FC = () => {
    ...
    const msgName = t('Welcome to Colouring') + ' ' + config.cityName + '. ' + t("You're one of the first people to sign up!");
    ...
    return (
        <article>
            <section className="main-col">
                ...
                <p>
                    {t('Create an account to start colouring in.')}
                </p>
                <ErrorBox msg={error} />
                <form onSubmit={onSubmit}>
                    ...
                    <input name="username" id="username"
                        ...
                        placeholder={t("not-your-real-name")} required
                        minLength={4}
                        maxLength={29}
                        pattern="\w+"
                    ...
                    />
                    <InfoBox msg={t("Our policy is to avoid collecting personal data wherever possible. An email address is only necessary if you wish to be able to recover your account should you forget your password.")} />
                    
                    <label htmlFor="confirm_email">{t('Confirm email (optional)')}</label>
                    <input name="confirm_email" id="confirm_email"
                        className="form-control" type="email"
                        value={confirmEmail} onChange={e => setConfirmEmail(e.target.value)}
                    />

                    <label htmlFor="password">{t('Password (at least 8 characters)')}</label>
                    ...
                    <div className="form-check">
                        ...
                        <label className="form-check-label" htmlFor="confirm_conditions">
                            {t('I confirm that I have read and agree to the')} <Link
                                to="/privacy-policy.html">{t('privacy policy')}</Link>, <Link
                                to="/contributor-agreement.html">{t('contributor agreement')}</Link> {t('and')} <Link
                                to="/data-accuracy.html">{t('data accuracy agreement')}</Link>.
                        </label>
                    </div>

                    <div className="buttons-container with-space">
                        <input type="submit" disabled={isLoading} value={t("Sign Up2")} className="btn btn-primary" />
                        {isLoading && <span><SpinnerIcon/> Sending sign up data...</span>}
                    </div>
                    <InfoBox msg="">
                        {t('Please also read our')} <a href="https://www.pages.colouring.london/data-ethics">{t('data ethics policy')}</a> {t('before using or sharing our data')}
                    </InfoBox>

                    {t('Do you already have an account?')}

                    <div className="buttons-container with-space">
                        <Link to="login.html" className="btn btn-outline-dark">{t('Log in')}</Link>
                    </div>
